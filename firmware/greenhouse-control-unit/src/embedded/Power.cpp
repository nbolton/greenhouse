...
bool Power::batteryIsLow() const {
  ...
  return !vBattKnown || (vBattOffKnown &&
...
bool Power::batteryIsCharged() const {
  ...
  return vBattKnown && vBattOnKnown &&
...
void Power::Loop() {
  ...
  const bool psuVoltageChanged =
      abs(m_lastPsuVoltage - psuVoltage) > VOLTAGE_DIFF_DELTA;
  if (psuVoltageChanged) {
    TRACE_F(TRACE_DEBUG1, "PSU voltage changed, was %.2fV, now %.2fV",
            m_lastPsuVoltage, psuVoltage);

#if POWER_EN
    if ((m_source == PowerSource::k_powerSourcePsu) &&
        (psuVoltage < k_psuVoltageMin)) {
  ...
  if (batteryVoltageChanged) {
    ...
    if ((m_source == PowerSource::k_powerSourceBattery) &&
        (batteryVoltage < k_batteryVoltageMin)) {
...
void Power::MeasureVoltage() {
  ...
  if (m_batteryVoltageSensor != k_unknown) {
    m_batteryVoltageOutput = mapFloat(m_batteryVoltageSensor, VS_IN_MIN,
                                      VS_IN_MAX, VS_OUT_MIN, VS_OUT_MAX);
  } else {
    m_batteryVoltageOutput = k_unknown;
  }
  ...
  if (m_psuVoltageSensor != k_unknown) {
    m_psuVoltageOutput = mapFloat(m_psuVoltageSensor, VS_IN_MIN, VS_IN_MAX,
                                  VS_OUT_MIN, VS_OUT_MAX);
  } else {
    m_psuVoltageOutput = k_unknown;
  }
}
...
void Power::MeasureCurrent() {
  TRACE(TRACE_DEBUG2, "Measuring current");
  ...
  TRACE_F(  //
      TRACE_DEBUG2,
      "Current result: shunt=%.2f, bus=%.2f, current=%.2f, power=%.2f, "
      "load=%.2f",
      shunt, bus, current, power, load);
...
void Power::switchSource(PowerSource source) {
  if (source == PowerSource::k_powerSourcePsu) {
    ...
    if (m_psuVoltageOutput < k_psuVoltageMin) {
      TRACE_F(TRACE_DEBUG1, "Unable to use PSU, voltage too low: %.2fV",
              m_psuVoltageOutput);
    ...
    if (batteryIsLow()) {
      TRACE_F(TRACE_DEBUG1, "Unable to use battery, voltage too low: %.2fV",
  ...
  Embedded().WriteOnboardIO(
      IO_PIN_PSU_SW, source == k_powerSourcePsu ? SWITCH_ON : SWITCH_OFF);
  Embedded().WriteOnboardIO(
      IO_PIN_BATT_SW, source == k_powerSourceBattery ? SWITCH_ON : SWITCH_OFF);

  Embedded().WriteOnboardIO(IO_PIN_PSU_LED,
                            source == k_powerSourcePsu ? LED_ON : LED_OFF);
  Embedded().WriteOnboardIO(IO_PIN_BATT_LED,
                            source == k_powerSourceBattery ? LED_ON : LED_OFF);
